...
public class ScanOnlyModeManagerTest {
    ...
    private ScanOnlyModeManager createScanOnlyModeManager() throws Exception {
        when(mClientInterface.asBinder()).thenReturn(mClientInterfaceBinder);
        ScanOnlyModeManager newScanOnlyModeManager = new ScanOnlyModeManager(mContext,
                mLooper.getLooper(), mWifiNative, mListener, mClientInterface, mNmService,
                mWifiMonitorMock.getWifiMonitor());
        return newScanOnlyModeManager;
    }

    /** Verifies startScanOnlyMode . */
    @Test
    public void startScanOnlyMode() throws Exception {
        startScanOnlyModeAndVerifyEnabled();
        verifyScanOnlyModeNotDisabled();
    }

    /**
     * Variant of startScanOnlyMode test where interface up notification comes in after supplicant
     * is connected.
     *
     * This test starts ScanOnlyMode and verifies that it is enabled successfully.
     */
    @Test
    public void startScanOnlyModeInterfaceUpAfterSupplicantUp() throws Exception {
     ...
     *
     * Start ScanOnlyMode except return false for enableSupplicant.
     * Expectation: WifiListener should be called with an updated state of WIFI_STATE_UNKNOWN.
     */
    @Test
    public void testStartScanOnlyModeSupplicantStartFails() throws Exception {
        mScanOnlyModeManager = createScanOnlyModeManager();
        when(mWifiNative.startHal()).thenReturn(true);
    ...
    /**
     * Test ScanOnlyModeManager when attempting to start supplicant throws a RemoteException.
     *
     ...
     */
    @Test
    public void testStartScanOnlyModeSupplicantStartThrowsRemoteException() throws Exception {
        mScanOnlyModeManager = createScanOnlyModeManager();
        when(mWifiNative.startHal()).thenReturn(true);
    ...
    /**
     * Test ScanOnlyModeManager when registerObserver throws a remote exception.
     *
     * Expectation: The exception should be caught and trigger cleanup of the startup state that has
     * completed.
     */
    @Test
    public void startScanOnlyModeRegisterObserverThrowsException() throws Exception {
        mScanOnlyModeManager = createScanOnlyModeManager();
        when(mWifiNative.startHal()).thenReturn(true);
        when(mClientInterface.enableSupplicant()).thenReturn(true);
    ...
    /**
     * Tests ScanOnlyMode startup if fail to link interface DeathRecipient.
     *
     ...
     */
    @Test
    public void startScanOnlyModeFailToLinkDeathRecipient() throws Exception {
        mScanOnlyModeManager = createScanOnlyModeManager();
    ...
    /** Tests the handling of stop command when ScanOnlyMode is not started. */
    @Test
    ...
    /** Tests the handling of stop command when ScanOnlyModeManager is started. */
    @Test
    public void stopWhenStarted() throws Exception {
        startScanOnlyModeAndVerifyEnabled();
        // clear expectations from the verification for starting ScanOnlyMode on mContext
        reset(mContext);
    ...
    /**
     * Test ScanOnlyModeManager in started state will process wificond interface death and tell
    ...
    /**
     * Test ScanOnlyModeManager handles the interface down notification when already started.
     *
     * Expectation: ScanOnlyModeManager should exit the started state and inform ScanningService
    ...
    /**
     * Test ScanOnlyModeManager ignores a CMD_STOP in idle mode.
     *
     * Expectation: ScanOnlyModeManager should do make extra calls to clean up if the interface down
    ...
    /**
     * Starts ScanOnlyMode and verifies that it is enabled successfully.
     *
     * This method creates a ScanOnlyManager and verifies that the broadcasts for supplicant
     * connected and scanning available are send.
     */
    protected void startScanOnlyModeAndVerifyEnabled() throws Exception {
        InOrder order = inOrder(mListener, mClientInterfaceBinder, mClientInterface,
                mNmService, mContext);
        ArgumentCaptor<Intent> intent = ArgumentCaptor.forClass(Intent.class);
    ...
    /**
     * Verifies that ScanOnlyMode was not disabled (assuming it was started).
     */
    protected void verifyScanOnlyModeNotDisabled() throws Exception {
