...
public class WifiPermissionsUtilTest {
    ...
    @Mock private WifiPermissionsWrapper mMockWpi;
    ...
    private static final int NON_LEGACY = 4;
    ...
    private static final int MAX_PERMISSIONS = 11;
    private static final int NUM_TEST_CASES = 15;
    ...
    */
    ...
    private void setupPermissionStrings() {
        ...
        sPermissions[NON_LEGACY] = "notLegacyVersion";
    ...
    private void setupTestCases() {
        ...
        sTestVector[index++] = ((0x1 << IS_ACTIVE_NW_SCORER)
            | (0x1 << IS_FOREGROUND_APP)
            | (0x1 << WIFI_SCAN_ALLOWED_APPS)
            | (0x1 << IS_USER_CURRENT));
        sTestVector[index++] = ((0x1 << IS_FOREGROUND_APP)
            | (0x1 << WIFI_SCAN_ALLOWED_APPS)
            | (0x1 << IS_USER_CURRENT));
        sTestVector[index++] = ((0x1 << IS_FOREGROUND_APP)
            | (0x1 << WIFI_SCAN_ALLOWED_APPS)
        ...
        sTestVector[index++] = ((0x1 << IS_FOREGROUND_APP)
            | (0x1 << NON_LEGACY));
        sTestVector[index++] = ((0x1 << IS_LOCATION_ENABLED)
    ...
    @Test public void test() throws Exception {
        for (int i = 0; i < NUM_TEST_CASES; i++) {
            ...
            mCodeUnderTest = new WifiPermissionsUtil(mMockWifiInjector);
            boolean output = mCodeUnderTest.canAccessScanResults(sPkgName);
           /**
    ...
    private void setupMocks() throws Exception {
        ...
        when(mMockWifiInjector.getWifiPermissionsWrapper())
            .thenReturn(mMockWpi);
    }
    private void setupTestVars(int testCase) {
        ...
        if (getValAtOffset(testCase, NON_LEGACY)) {
    ...
    private void setupMockInterface(int testCase) {
        when(mMockWpi.getCallingUserId()).thenReturn(mCallingUser);
        when(mMockWpi.getCurrentUser()).thenReturn(mCurrentUser);
        when(mMockWpi.getUid()).thenReturn(mUid);
        when(mMockWpi.isCallerActiveNwScorer())
            .thenReturn(getValAtOffset(testCase, IS_ACTIVE_NW_SCORER));
        when(mMockWpi.getUidPermission(mManifestStringFine))
            .thenReturn(mFineLocationPermission);
        when(mMockWpi.getUidPermission(mManifestStringCoarse))
            .thenReturn(mCoarseLocationPermission);
        when(mMockWpi.getLocationModeSetting())
            .thenReturn(mLocationModeSetting);
        when(mMockWpi.getUserManager()).thenReturn(mMockUserManager);
        when(mMockWpi.getTopPkgName()).thenReturn(mPkgNameOfTopActivity);
        when(mMockWpi.getContext()).thenReturn(mMockContext);
    }
    private static boolean computeResult(int input) {
        boolean isLegacyForeground = (!getValAtOffset(input, NON_LEGACY)
