...
public class ANQPRequestManagerTest {
     ...
     * the previous request results in a failure.  The request will succeed when the hold off time
     * is up.
     *
     * @throws Exception
     */
    @Test
    public void requestANQPElementsAfterFailedRequest() throws Exception {
        ...
        // Attempt another request will fail while the hold off time is not up yet.
        when(mClock.getElapsedSinceBootMillis()).thenReturn(startTime + 1);
        assertFalse(mManager.requestANQPElements(TEST_BSSID, TEST_ANQP_KEY, false, false));
        verify(mHandler, never()).requestANQP(anyLong(), anyObject());
        reset(mHandler);
    ...
    /**
     * Verify the hold off time for each unanswered query, and it will stay the same after
     * reaching the max hold off count {@link ANQPRequestManager#MAX_HOLDOFF_COUNT} is reached.
     *
     * @throws Exception
     */
    @Test
