...
Return<void> WifiChipService::pollChipInfo() {
  if (!hal_service_) return Void();

  IWifiChipEventCallback::ChipInfo result;
  result.driverVersion = "<unknown>";
  ...
  for (auto& callback : callbacks_) {
    callback->onChipInfoAvailable(result);
  }
  return Void();
}
