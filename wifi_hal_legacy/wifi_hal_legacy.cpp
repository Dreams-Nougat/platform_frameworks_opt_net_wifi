...
WifiHalLegacy* WifiHalLegacy::global_instance_;

WifiHalLegacy::WifiHalLegacy(android::sp<android::Looper>& looper) :
...
Status WifiHalLegacy::registerEventCallback(
    const sp<IWifiEventCallback>& callback) {
  callbacks_.insert(callback);
  return Status::ok();
}

Status WifiHalLegacy::start(uint32_t cmd_id) {
  ...
  LOG(ERROR) << "Initializing HAL";
  wifi_error status = hal_func_table_.wifi_initialize(&hal_handle_);
  if (status == WIFI_SUCCESS) {
    // TODO should thread creation failure be handle gracefully or should we
    // just crash the process
    event_loop_thread_ = std::thread(&WifiHalLegacy::DoHalEventLoop, this);
...
Status WifiHalLegacy::stop(uint32_t cmd_id) {
  ...
  LOG(INFO) << "HAL cleanup command complete";
  FinishHalCleanup();
  return Status::ok();
}

void WifiHalLegacy::DoHalEventLoop() {
  LOG(INFO) << "Starting HAL event loop";
  hal_func_table_.wifi_event_loop(hal_handle_);
  LOG(INFO) << "HAL Event loop terminating";
  if (state_ != State::STOPPING) {
    // TODO what happens if the event loop terminates and were not stopping?
    // How do we tear down? just crash?
  }
...
void WifiHalLegacy::FinishHalCleanup() {
...
void WifiHalLegacy::PostTask(const std::function<void()>& callback) {
