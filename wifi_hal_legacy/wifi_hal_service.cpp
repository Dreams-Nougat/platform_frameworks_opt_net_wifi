...
Status WifiHalService::registerEventCallback(
    const sp<IWifiEventCallback>& callback) {
  callbacks_.insert(callback);
  return Status::ok();
}

Status WifiHalService::start(uint32_t cmd_id) {
...
Status WifiHalService::stop(uint32_t cmd_id) {
...
void WifiHalService::DoHalEventLoop() {
  ...
  LOG(VERBOSE) << "HAL Event loop terminating";
