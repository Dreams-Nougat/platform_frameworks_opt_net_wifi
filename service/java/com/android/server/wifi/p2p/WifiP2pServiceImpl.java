...
public class WifiP2pServiceImpl extends IWifiP2pManager.Stub {
    ...
    private static int sLatestBuildVersionOrO = getLatestOrOBuildVersoin();
    ...
    /**
     * This function returns a version code corresponding to O, if present
     * Or the latest build version number currently available.
     * @return int build version number
     */
    private static int getLatestOrOBuildVersoin() {
        int version = -1;
        try {
            // If O is present, this will correspond to its build number
            version = Build.VERSION_CODES.class.getField("O").getInt(null);
        } catch (Exception e) {
            // This it the current version number of the latest support build
            version = Build.VERSION.RESOURCES_SDK_INT;
        }
        return version;
    }
    ...
    private ClientInfo getClientInfo(Messenger m, boolean createIfNotExist) {
        ...
        private WifiP2pDeviceList getPeers(Bundle pkg, int uid) {
            ...
            try {
                scanPermission = wifiPermissionsUtil.canAccessScanResults(pkgName,
                        uid, sLatestBuildVersionOrO);
