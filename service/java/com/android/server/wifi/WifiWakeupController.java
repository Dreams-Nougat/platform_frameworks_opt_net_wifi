...
class WifiWakeupController {
    ...
    private void handleScanResultsAvailable() {
        ...
        if (mWifiState != WifiManager.WIFI_STATE_DISABLED) {
            Log.w(TAG, "Wifi already enabled.");
            return;
        }
        ...
        for (int i = 0; i < scanResults.size(); i++) {
            ...
            if (mSavedSsidsOnDisable.contains(scanResult.SSID)) {
                Log.w(TAG, "Not enabling wifi because ssid in range: " + scanResult.SSID);
                return;
            }
            scanDetails.add(ScanResultUtil.toScanDetail(scanResult));
        }
        ...
        if (selectedNetwork != null) {
            // TODO(b/): show notification for wifi enablement
