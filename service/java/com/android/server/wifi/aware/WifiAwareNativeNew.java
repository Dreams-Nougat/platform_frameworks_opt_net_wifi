...
public class WifiAwareNativeNew {
    ...
    public boolean isSupportedByCurrentMode(IWifiChip chip) {
        ...
        MutableInt currentModeId = new MutableInt(0);
        try {
            chip.getMode(new IWifiChip.getModeCallback() {
                ...
                public void onValues(WifiStatus status, int modeId) {
                    ...
                    if (statusOk.value) {
                        ...
                        Log.e(TAG,
                                "isSupportedByCurrentMode: getMode() error: " + status.description);
                    }
                }
            });
        } catch (Exception e) {
            Log.e(TAG, "isSupportedByCurrentMode: getMode() exception: " + e);
            return false;
        }
        if (!statusOk.value) {
            return false;
        ...
        ArrayList<IWifiChip.ChipMode> chipModes = new ArrayList<>();
        try {
            chip.getAvailableModes(new IWifiChip.getAvailableModesCallback() {
                @Override
                public void onValues(WifiStatus status, ArrayList<IWifiChip.ChipMode> modes) {
                    statusOk.value = status.code == WifiStatusCode.SUCCESS;
                    if (statusOk.value) {
                        chipModes.addAll(modes);
                    } else {
                        Log.e(TAG, "isSupportedByCurrentMode: getAvailableModesCallback() error: "
                                + status.description);
                    }
                }
            });
        } catch (Exception e) {
            Log.e(TAG, "isSupportedByCurrentMode: getAvailableModesCallback() exception: " + e);
            return false;
        }
