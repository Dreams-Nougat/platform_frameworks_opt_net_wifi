...
public class WifiMetrics {
    ...
    private static final int MAX_RSSI_DELTA = 127;
    private static final int MIN_RSSI_DELTA = -127;
    /** Maximum time period between ScanResult and RSSI poll to generate rssi delta datapoint */
    private static final long TIMEOUT_RSSI_DELTA_MILLIS =  3000;
     ...
     */
    private long mRecordStartTimeSec = 0;
    ...
    private void consolidateProto(boolean incremental) {
        ...
        synchronized (mLock) {
            ...
            for (int i = 0; i < mRssiDeltaCounts.size(); i++) {
                WifiMetricsProto.RssiDeltaCount keyVal = new WifiMetricsProto.RssiDeltaCount();
                keyVal.rssi = mRssiDeltaCounts.keyAt(i);
                keyVal.count = mRssiDeltaCounts.valueAt(i);
                rssis.add(keyVal);
            }
            mWifiLogProto.rssiDeltaRssiCount = rssis.toArray(mWifiLogProto.rssiDeltaRssiCount);
