...
public class WifiStateMachinePrime {
    ...
    private ModeActiveState mClientModeActiveState = new ClientModeActiveState();
    private ModeActiveState mScanOnlyModeActiveState = new ScanOnlyModeActiveState();
    private ModeActiveState mSoftAPModeActiveState = new SoftAPModeActiveState();
    ...
    WifiStateMachinePrime(WifiInjector wifiInjector, Looper looper) {
        Log.e(TAG, "setting mWifiInjector: "+ mWifiInjector + " " + wifiInjector);
    ...
    private boolean checkForAndHandleModeChange(Message message) {
        Log.e(TAG, "check message for mode switch in ModeState.processMessage");
        switch(message.what) {
            case ModeStateMachine.CMD_START_CLIENT_MODE:
                Log.e(TAG, "Switching from " + getCurrentMode() + " to ClientMode");
                mModeStateMachine.transitionTo(mClientModeState);
                break;
            case ModeStateMachine.CMD_START_SCAN_ONLY_MODE:
                Log.e(TAG, "Switching from " + getCurrentMode() + " to ScanOnlyMode");
                mModeStateMachine.transitionTo(mScanOnlyModeState);
                break;
            case ModeStateMachine.CMD_START_SOFT_AP_MODE:
                Log.e(TAG, "Switching from " + getCurrentMode() + " to SoftApMode");
                mModeStateMachine.transitionTo(mSoftAPModeState);
                break;
            case ModeStateMachine.CMD_DISABLE_WIFI:
                Log.e(TAG, "disabling wifi, calling exit on " + getCurrentMode());
    ...
    class ClientModeState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            Log.e(TAG, "In processMessage of ClientModeState: " + message);
            return NOT_HANDLED;
        }
    ...
    class SoftAPModeState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                case CMD_START_AP:
                    Log.e(TAG, "Transitioning to softapmodeactivestate");
                ...
                case CMD_START_AP_FAILURE:
                    Log.e(TAG, "Failed to start SoftApMode.  Waiting for next mode command.");
                    break;
                case CMD_AP_STOPPED:
                    Log.e(TAG, "SoftApModeActiveState stopped.  Waiting for next mode command.");
                    break;
                default:
                    return NOT_HANDLED;
            }
    ...
    class SoftAPModeActiveState extends ModeActiveState {
        ...
        @Override
        public void enter() {
            Log.e(TAG, "Entering SoftApModeActiveState");
            final Message message = mModeStateMachine.getCurrentMessage();
            Log.e(TAG, "Checking currentMessage: " + message.what + " ?= " + CMD_START_AP);
        ...
        public boolean processMessage(Message message) {
            switch(message.what) {
                case CMD_START_AP:
                    Log.e(TAG, "Received CMD_START_AP with SoftApMode already active.  drop");
                case CMD_STOP_AP:
                    mActiveModeManager.stop();
                    break;
                case CMD_START_AP_FAILURE:
                    Log.e(TAG, "Failed to start SoftApMode.  return to SoftApMode (inactive).");
                    mModeStateMachine.transitionTo(mSoftAPModeState);
                    break;
                case CMD_AP_STOPPED:
                    Log.e(TAG, "SoftApModeActiveState stopped.  return to SoftApMode (inactive).");
                    mModeStateMachine.transitionTo(mSoftAPModeState);
                    break;
                default:
                    return NOT_HANDLED;
    ...
    private void changeMode(int newMode) {
        if (mModeStateMachine == null) {
            ...
            Log.e(TAG, "already in: " + getCurrentMode()
                    + " - switch to: " + mCommandToStateMap.get(newMode));
