...
public class WifiAsyncChannel extends AsyncChannel {
    ...
    private WifiLog fetchOrInitLog() {
        // Since mLog is initialized the first time this service
        // initiates a function using mLog to log message over
        // this Async Channel, check if mLog is null and if so, it
        // is initialized. Otherwise, mLog is already initialized
        // for this AsyncChannel
        if (mLog == null) {
            WifiInjector wifiInjector = WifiInjector.getInstance();
            mLog = wifiInjector.makeLog(mTag);
        }
        return mLog;
    }
    ...
    public void sendMessage(Message msg) {
        fetchOrInitLog().trace("sendMessage,message=% sendingUid=%")
            .c(msg.what)
            .c(msg.sendingUid)
            .flush();
    ...
    public void replyToMessage(Message srcMsg, Message dstMsg) {
        fetchOrInitLog()
                .trace("replyToMessage,message=% sendingUid=% with message=%")
                .c(srcMsg.what)
                .c(srcMsg.sendingUid)
                .c(dstMsg.what)
                .flush();
    ...
    public Message sendMessageSynchronously(Message msg) {
        fetchOrInitLog().trace("sendMessageSynchronously,message=% sendingUid=%")
            .c(msg.what)
            .c(msg.sendingUid)
            .flush();
        Message replyMessage = super.sendMessageSynchronously(msg);
        fetchOrInitLog().trace("recvdMessageSynchronously,message=% sendingUid=%")
            .c(replyMessage.what)
