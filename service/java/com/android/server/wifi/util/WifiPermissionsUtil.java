...
public class WifiPermissionsUtil {
    ...
    public boolean canAccessScanResults(String pkgName, int uid,
        ...
        // If the caller has neither of the above, return false
        if (!canCallerAccessLocation && !canSystemUseLocation) {
            return false;
        }
        // check if Wifi Scan request is allowed operation for this App
        if (!isScanAllowedbyApps(pkgName, uid)) {
            return false;
        }
        // If the user or profile is not current, check if uid has
        // INTERACT_ACROSS_USERS_FULL permission. Otherwise, return false
