...
public class WifiPermissionsUtil {
    ...
    public boolean canAccessScanResults(String pkgName, int uid,
        ...
        // permission or is an Active Nw scorer or has enabled
        // LocationMode (For AppVersion older than minVersion,
        // it is sufficient to check if the App is foreground.
        // Othewise, Location Mode must be enabled), continue
        // to check other permissions. Otherwise, return false.
        ...
        // If the caller has CAN_READ_PEER_MAC_ADDRESS
        // permission or is an Active Nw scorer or has Location
        // Permission (Coarse Permission implies Fine Permission)
        // Note: Location Permission is bypassed for App version older
        // than minVersion if the app in the foreground. If true,
        // then proceed to check other permissions. If none of the
        // above are true, then deny access to scan results
        if (!checkCallerHasPeersMacAddressPermission(uid)
                && !isCallerActiveNwScorer(uid)
        ...
        // If the user or profile is current, return true. Otherwise return
        // true if the uid has INTERACT_ACROSS_USERS_FULL permission
