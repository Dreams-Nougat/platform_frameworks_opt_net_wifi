...
public class WifiPermissionsUtil {
     ...
     */
    public boolean canAccessScanResults(String pkgName, int uid, int pid,
        ...
        if (!checkCallerHasPeersMacAddressPermission(uid, pid)
        ...
        if (!checkInteractAcrossUsersFull(uid, pid)) {
            return false;
        }
        return true;
    }
     ...
     */
    private boolean checkCallerHasPeersMacAddressPermission(int uid, int pid) {
        return mContext.checkPermission(
                android.Manifest.permission.PEERS_MAC_ADDRESS, uid, pid)
     ...
     */
    private boolean checkInteractAcrossUsersFull(int uid, int pid) {
        return (mContext.checkPermission(
                android.Manifest.permission.INTERACT_ACROSS_USERS_FULL, uid, pid)
                == PackageManager.PERMISSION_GRANTED);
    }
    ...
    private boolean checkCallersLocationPermission(String pkgName, int uid, int version) {
        ...
        // Location permissions is granted for apps older than version if foreground
