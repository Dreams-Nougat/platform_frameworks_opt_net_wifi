...
public class WifiNetworkSelector {
    ...
    private boolean isCurrentNetworkQualified(WifiConfiguration network) {
        // Currently connected?
        if (network == null) {
    ...
    private boolean isNetworkSelectionNeeded(List<ScanDetail> scanDetails,
        ...
        if (connected) {
            ...
            if (isCurrentNetworkQualified(mCurrentNetwork)) {
                localLog("Current connected network is already qualified. Skip network selection.");
                return false;
            } else {
