...
public final class WifiMeshServiceImpl extends IWifiMeshManager.Stub {
    ...
    private static final String AUTH_PROT_STR = "authentication_protocol_id=0x";
    ...
    private class MeshStateMachine extends StateMachine {
            ...
            public boolean processMessage(Message message) {
                ...
                case WifiStateMachine.CMD_ENABLE_MESH_REQ:
                    try {
                        ...
                        mNwService.setInterfaceIpv6PrivacyExtensions(mInterface, true);
        ...
        class MeshJoinedState extends State {
            private LinkAddress getStaticIpAddress(String hwAddr) {
                int value4 = Integer.parseInt(
                    hwAddr.substring(hwAddr.length() - 5, hwAddr.length() - 3), 16);
                int value5 = Integer.parseInt(
                    hwAddr.substring(hwAddr.length() - 2), 16);
                if (value5 == 0) {
                    value5 = 2;
                } else if (value5 == 255) {
                    value5 = 3;
                ...
                return new LinkAddress("172.16." + value4 + "." + value5 + "/16");
            }

            private InetAddress getStaticGatewayAddress() throws UnknownHostException {
                byte addr[] = new byte[]{ (byte)172, (byte)16, (byte)11, (byte)1 };
            ...
            public void enter() {
                Log.i(TAG, "Enter " + getName());
                /*
                 * XXX: Disable DHCP for static IP address assignment.
                ...
                */
                ...
                try {
                    ifcg = mNwService.getInterfaceConfig(mInterface);
                } catch (RemoteException re) {
                    loge("getInterfaceConfig failed: " + re);
                    return;
                }

                StaticIpConfiguration config = new StaticIpConfiguration();
                config.ipAddress = getStaticIpAddress(ifcg.getHardwareAddress());
                ifcg.setLinkAddress(config.ipAddress);
                ifcg.setInterfaceUp();

                try {
                    config.gateway = getStaticGatewayAddress();
                    config.dnsServers.add(getStaticGatewayAddress());
                    mNwService.setInterfaceConfig(mInterface, ifcg);
                } catch (UnknownHostException ex) {
                    loge("Generate IP address failed: " + ex);
                    return;
                } catch (RemoteException re) {
                ...
                try {
                    mNwService.addInterfaceToLocalNetwork(mInterface,
                        config.getRoutes(mInterface));
                } catch (RemoteException re) {
                    loge("addInterfaceToLocalNetwork failed: " + re);
                }
            }
            ...
            public boolean processMessage(Message message) {
                ...
                case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                    ...
                    if (message.arg1 == DhcpStateMachine.DHCP_SUCCESS &&
                        ...
                        try {
                            mNwService.addInterfaceToLocalNetwork(mInterface,
                                dhcpResults.getRoutes(mInterface));
            ...
            public void exit() {
                ...
                try {
                    mNwService.removeInterfaceFromLocalNetwork(mInterface);
                } catch (RemoteException e) {
        ...
        private boolean connect(int netId, WifiMeshGroup group) {
            ...
            if (group.isSecure()) {
                mWifiNative.setNetworkVariable(netId, "psk",
                    "\"" + group.getPassphrase() + "\"");
            }
            mWifiNative.setNetworkVariable(netId, "mode", "5");
        ...
        private void setScanResults() {
            ...
            while (true) {
                tmpResults = mWifiNative.scanResults(sid, "0x69D87");
            ...
            int freq = -1, level = -100, auth_prot = 0;
            long tsf = 0;
            ...
            for (String line : lines) {
                if (line.startsWith(BSSID_STR)) {
                ...
                } else if (line.startsWith(AUTH_PROT_STR)) {
                    auth_prot = Integer.parseInt(line.substring(AUTH_PROT_STR.length()), 16);
                    ...
                    if (bssid != null && mesh_id != null) {
                        mScanResults.update(new ScanResult(
                            ...
                            "[MESH]" + (auth_prot == 0 ? "" : "[SAE]"), level, freq, tsf));
                    }
                    bssid = mesh_id = null;
                    level = freq = 0;
                    tsf = 0;
